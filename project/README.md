...
That means that for a person `p` , we known that probably that person contracted the virus from `p_j` where `j` is the population of the country reporting contamination cases.
...
4- Implement (write code!) a first naive solution but one that pass all tests (make something simple, but which works and build from that). 
