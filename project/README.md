...
Especially, we are asked by the european union parlment to report the top-3 more important chain of contaminated persons we can find in the provided datasets any time a new case is reported. 
...
Between 7 days (168 hours, exclusive) and 14 days (336 jours, inclusive), the case is considered with an importance score of 4.
After 14 days (336 jours, exclusive), that case is considered 0 (although it can still participate to a chain of contamination if the overall score of that chain is not 0).
...
person_age : age in years of that person (floored, e.g. 23 years, 10 months 3 days old person will be reccorded as 23 years old) 
diagnosed_ts : diagnosed time of coronavirus infection (supposed datetime when the person was suppoosed to be infected).
...
Why include it then ? Remember that these are fictional data: this field is added in this fictional toy project for you to more easily read and debug your pogram.
...
At any new reccorded cases, we want to output the top-3 longest contagious chain as follows:
...
This is the first event from Spain that must be poped out of the remaining events.
...
Consequently, it shall be considerd as a root case for a new contamination chain (illustration of Rule 4).
This event happens three days after the last Italian case therefore they both remain at a score of 10. 
When there is a draw, the oldest chain comes first (ilustration of Rule 6).
Therefore, after this event is processed, the output file contains :
```
France, 1
France, 1
...
As always, we want FIRST a program that we are sure to be correct through unit testing, and only SECOND optimsations.
...
_Finally, it is obvious but better made explciit that teachers will take into account for passing the difficulties raised by not being altogether in the same classroom._
