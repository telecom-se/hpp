...
- Nommage des intrinsics (=fonctions C traduites directement en 1 instruction ASM)
  https://software.intel.com/en-us/cpp-compiler-developer-guide-and-reference-naming-and-usage-syntax
...
- Alignement mémoire : quand on utilise les instructions vectorielles SSE/AVX, il est généralement nécessaire (ou, des fois, simplement plus rapide) que les données soient alignées en mémoire.
  - Pour forcer l'alignement, on peut utiliser l'instruction "alignas"
...
> Utilisez la documentation Intel pour chercher les intrinsics SSE permettant (1) de charger 4 float dans un registre, (2) d'additionner 4 floats et (3) multiplier 4 floats
  - https://software.intel.com/sites/landingpage/IntrinsicsGuide/
> Complétez le code proposé avec les intrinsics trouvés pour calculer a*b+c des trois façons demandées (C, SSE, FMA)
> À votre avis, à quoi peut servir une telle instruction a*b+c ?
> Utiliser une "high_resolution_clock" pour évaluer le temps d'exécution des 3 fonctions
...
- Utilisez les resources ci-dessus pour trouver le/les bons headers à inclure
- Écrivez une fonction pour remplir une matrice ayant ce format. Un booléen est fourni en entrée. S'il est à vrai, la matrice est remplie de 0, sinon elle est remplie de valeurs aléatoires.
- Écrivez une fonction pour afficher une telle matrice.
- Écrivez une fonction pour multiplier deux matrices et stocker le résultat dans une 3ème matrice passée en paramètre, sans chercher à optimiser pour le moment
...
- Terminez le main() pour tester vos fonctions et vérifier que vous optenez bien les mêmes résultats selon les 3 méthodes.
- Regardez ce que le compilateur de VisualStudio avait généré dans le cas 1 et comparez à votre propre code :
...
- Recherchez sur internet comment on pourrait demander à VisualStudio d'améliorer le code qu'il génère.
  - Tester ce que vous avez trouvé
- Si vous avez le temps, ajouter des instructions pour comparer les temps d'exécution sur de grosses matrices
...
- Buts :
  ...
  - Dessiner l'ensemble de Mandelbrot, selon l'agorithme "Escape Time"
...
- On vous donne :
  ...
  - NOTE: le projet est configuré pour utiliser OpenCV
...
- Écrivez une version de la fonction en utilisant 2 floats à la place d'objets <complex> pour représenter les nombres complexes.
- Utilisez la documentation Intel pour ré-écrire cette version en utilisant au maximum des instructions SSE
  - https://software.intel.com/sites/landingpage/IntrinsicsGuide/
- Utilisez la documentation Intel pour ré-écrire cette version en utilisant au maximum des instructions AVX
- Utilisez le code de test donné dans main() pour tester vos fonctions
- Si vous avez le temps, ajouter des instructions pour comparer les temps de génération des images selon les différentes méthodes.
...
*  - NB_CHANTYPE_IN_128    : le nombre d'éléments de type chanType que l'on peut stocker dans un registre SSE (__m128) = 128bits/sizeof(float=4bytes) = 4 floats = 4 channels = 1 pixel
*  - NB_CHANTYPE_IN_256    : le nombre d'éléments de type chanType que l'on peut stocker dans un registre AVX (__m256) = 256bits/sizeof(float=4bytes) = 8 floats = 8 channels = 2 pixels
...
- Intuitivement, à quels gains de performance pourrait-on s'attendre ?
- Écrivez les méthodes fillWithZero(), fillWithZeroOptimized(), fillWithZeroSSE(), fillWithZeroAVX() pour remplir une image de 0.
...
- Écrivez les méthodes fillWithColor(), fillWithColorOptimized(), fillWithColorSSE(), fillWithColorAVX() pour remplir une image avec la couleur définie par ImageManipulation::setTargetColor().
- Vérifier que vos codes fonctionnent avec le main() fournit dans VisualTesting.cpp
- Comparer les temps d'exécution des différentes méthodes. Que constatez-vous ?
  - Pour ce faire, exclure VisualTesting.cpp du projet et inclure TimeTesting.cpp 
- Écrivez les méthodes fadeToZero() et  fadeToZeroAVX(), dont le but est d'effectuer une transition d'une couleur à une autre en fonction du paramètre défini par setAlpha()
  - Notre que le fading vers 0 revient simplement à multiplier la couleur actuelle par alpha.
- Écrivez les méthodes fadeToColor() et fadeToColorAVX(), qui transitionnent cette fois-ci vers la couleur donnée par setTargetColor() et utilise le même paramètre défini par setAlpha()
- Écrivez les méthodes fadeToImage() et fadeToImageAVX(), qui transitionnent chaque pixel d'une image source vers le pixel correspondant d'une image destination. Ces méthodes supposent que les 2 images sont de mêmes dimensions et utilisent setAlpha() et setImageDest()
- Que se passerait-il si le format d'image était CV32FC3?
...
- En vous inspirant de la vidéo suivante et des réponses ci-dessus, expliquez pourquoi est-il toujours nécessaire d'intervenir manuellement pour optimiser l'implémentation d'un algorithme avec SIMD/SSE/AVX
